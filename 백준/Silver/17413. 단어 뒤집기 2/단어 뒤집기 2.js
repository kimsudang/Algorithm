...
for (let str of input) {
  if (str === "<") {
      ...
      isTag = false;
      // 태그가 끝났으므로 word에 넣어둔 문자열들과 함께 answer에 넣어준다.
      answer += word + str;
      // 다음 문자를 받아서 처리하기 위해 word를 초기화한다.
      word = "";
  } else if (str === " ") {
      // 공백이 있다면, 태그 내 공백인지, 문자 사이의 공백인지 판단해야한다.
      // 만약, 태그 내 공백이라면 그대로 word와 함께 공백 문자를 answer에 추가한다.
...
// 만약 ">" 기호로 태그가 닫히는 것으로 끝나는게 아니라, 문자로 끝나는 경우에는 word에 넣어진 상태로 for문 종료
